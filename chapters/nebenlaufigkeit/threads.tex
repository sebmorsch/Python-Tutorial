...
Wird kein Wert übergeben, übernimmt der Thread standardmäßig den Werte des erzeugenden Threads.
...
Nun wechselt die aktive Ausfürhung zurück zum ersten Thread, welcher noch den alten \lstinline$count$-Wert gelesen hat.
...
Da die \lstinline$Lock$-Klasse das Context-Management-Protokoll unterstützt, kann das Gleiche mit dem
...
wurde, wie \lstinline$aquire()$, und der interne Zähler wieder auf 0 steht.
...
\lstinputlisting[language=Python,firstline=6,lastline=29]{chapters/nebenlaufigkeit/src/beispiel_parkhaus.py}
...
\lstinputlisting[language=Python,firstline=32,lastline=54]{chapters/nebenlaufigkeit/src/beispiel_parkhaus.py}

Es wird nun ein neues \lstinline$CarPark$-Objekt erzeugt, das 5 Parkplätze besitzt.
Anschließend werden 10 \lstinline$Car$-Objekte initialisiert und gestartet.
...
\lstinputlisting[language=Python,firstline=57,lastline=60]{chapters/nebenlaufigkeit/src/beispiel_parkhaus.py}
...
kann ist im folgenden Beispiel gezeigt \cite{pythondokuthreads}.
Hier wird eine beschränkung von maximal 5 gleichzeitigen Zugriffen gesetzt.
Das \lstinline$with$-Statement ruft wieder automatisch \lstinline$aquire()$ und \lstinline$release()$ auf.
Somit ist garantiert, dass sich immer maximal 5 Threads innerhalb des \lstinline$with$-Statements 
befinden.

\label{threads:lst:semaphore}
\lstinputlisting[language=Python,firstline=3,lastline=15]{chapters/nebenlaufigkeit/src/beispiel_semaphore.py}
...
wartenden Threads wieder aufgeweckt und die \lstinline$Barrier$ wird in einen ''zerstörten''
Zustand gebracht.
...
Durch Aufruf von \lstinline$abort()$ wird die \lstinline$Barrier$ in einen ''zerstörten'' Zustand überführt.
...
Das Attribut \lstinline$broken$ gibt an, ob sich die \lstinline$Barrier$ im ''zerstörten'' Zustand befindet.

\tip{
Tritt in einem Thread ein Problem auf, und er kann nicht ordnugsgemäß fortgesetzt werden, kann
