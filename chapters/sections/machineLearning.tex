...
\textbf{pandas}\\
Die Webseite zum \textit{pandas}-Paket~\cite{pandas} beschreibt dieses als gute Wahl zur schnellen und flexiblen Aufbereitung von Daten. \textit{pandas} bietet verschiedene Möglichkeiten, um schnell auf Einträge zuzugreifen. Dies ist möglich, da mittels \textit{pandas} Serien und Dataframes erzeugt werden können, die im Gegensatz zu einem Array in Python auch Spaltentitel und Indexes können.

Die \textit{describe()}-Methode bietet hierbei einen ersten Überblick über die im Dataframe enthalten Daten. Ohne weitere Programmierung werden Informationen wie Maximalwert, Minimalwert und Durchschnitte für jede Spalte berechnet und angezeigt. 
...
\textbf{scipy}\\
Ergänzend bzw. aufbauen auf \textit{numpy} werden durch \textit{scipy} viele mathematische Operationen bereit gestellt. 
Das Modul \textit{scipy} ist sehr mächtig und daher nochmal in Untermodule aufgeteilt. Innerhalb der Untermodule werden bestimmte Funktionalitäten gruppiert. Eine Übersicht hierzu gibt die Onlinedokumentation~\cite{scipyreference}.
