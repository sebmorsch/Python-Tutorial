...
Um denn Fehlerfall zu betrachten, wurde die Rückgabe der Funktion von \lstinline$x*y$ auf \lstinline$x+y$ geändert.
...
Über \randnotiz{Strukturierung} das Modul \lstinline$doctest$ ist es einerseits möglich, Tests direkt an den Programmcode zu hängen und über den \lstinline$docString$ der Funktion Tests auszuführen.
Diese Tests werden am ende eines Moduls mit \lstinline$doctest.testmod()$ aufgerufen.
Eine weitere Möglichkeit ist über \lstinline$doctest.testfile()$ separate Testdateien festzulegen.
Es ist also möglich Tests zu einer Modul oder eine Klasse außerhalb zu definieren.
...
Das Modul erkennt automatisch das es in diesem fall \lstinline$doctest.testfile()$ nutzen muss.
...
Es kann beliebig viel Text in einer externen Testdatei stehen. 
Beachtet werden muss lediglich die Importanweisung des Moduls sowie das einleiten der einzelnen Testfälle über \lstinline$>>>$.