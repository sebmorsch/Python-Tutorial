...
\label{grundlagen:section:dateienlesenundschreiben}

Python bietet nativ Möglichkeiten für das Bearbeiten von Dateien. Hierfür werden Objekte erstellt und verwendet, die eine Datei im Quellcode repräsentieren. Mithilfe dieser, im folgenden \lstinline$fileObjects$ genannt, lässt sich der Inhalt einer Datei ändern und wird gespeichert, sobald es im Code mit der \lstinline$close()$-Methode geschlossen wird.

\subsection{Dateitypen}
\label{dateienlesenundschreiben:subsection:filetypes}
...
\label{dateienlesenundschreiben:subsection:open}
...
\label{dateienlesenundschreiben:lst:close}
Nach einem Schließen der Datei wird der verwendete Speicherplatz freigegeben. Das Arbeiten ist dann über das entsprechende \lstinline$fileObject$ nicht mehr möglich. 

\tip{Es empfiehlt sich direkt im Anschluss an die Verwendung der \lstinline$open()$-Methode die Datei an anderer Stelle zu schließen. Auf diese Weise wird sichergestellt, dass die Datei nicht unabsichtlich bearbeitet wird.}
...
\lstinline$"a"$: Hinzufügen von Inhalt am Ende der Datei. Erzeugt eine neue Datei, falls keine mit dem gewählten Namen an der angegebener Pfadangabe existiert.
...
\lstinline$"t","b"$: Angabe, ob die Datei als Text- \lstinline$"t"$ oder Binärdatei \lstinline$"b"$ interpretiert werden soll. Diese Modi können jeweils zu den anderen hinzugefügt werden. Standardmäßig wird die Datei als Text interpretiert, \lstinline$"t"$ kann hierbei weggelassen werden.
\lstinputlisting[language=Python, firstline=1,lastline=9]{chapters/basics/src/dateien_lesen_und_schreiben/FileHandling.py}
\label{dateienlesenundschreiben:lst:opentype}


\subsection{Methoden}
\label{dateienlesenundschreiben:subsection:methods}
...
\label{dateienlesenundschreiben:lst:openwrite}
...
\label{dateienlesenundschreiben:lst:openwritelines}
...
\label{dateienlesenundschreiben:lst:openread}
...
\label{dateienlesenundschreiben:lst:openreadfor}
...
\label{dateienlesenundschreiben:lst:openreadlines}
...
\label{dateienlesenundschreiben:lst:opentell}
...
\label{dateienlesenundschreiben:lst:openseek}

\subsection{With-Statement}
\label{dateienlesenundschreiben:subsection:withstatement}
...
\label{dateienlesenundschreiben:lst:openwithstatement}

\subsection{Attribute}
\label{dateienlesenundschreiben:subsection:attributes}
...
\label{dateienlesenundschreiben:lst:openattributes}